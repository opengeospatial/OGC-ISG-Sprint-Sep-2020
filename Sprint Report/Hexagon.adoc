...
=== On the fly vertex displacement: a solution to handle CDB model updates
...
terms of visual appearance and performance. In
addition, model updates may originate from other sources than the CDB store itself.

On the fly vertex displacement offers a solution for small model updates.
