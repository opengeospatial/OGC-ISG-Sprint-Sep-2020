...
InfoDao stood up a test server to quickly simulate server client transactions for consuming the data. The OGC API compliant PyGeoServer instance was configured to serve San Diego CDB data through a STAC interface. Since the InfoDao client could read CDBs and the dataset was easily traversable through the server's json responses, various operations of accessing and retrieving CDB data could be performed. The operations were:
